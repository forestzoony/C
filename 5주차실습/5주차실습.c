...
//학번: 20204024
//이름: 임지윤
//프로그램명: 이진 탐색 트리를 이용한 영어 사전 프로그램
...
TreeNode* search(TreeNode* root, element key)
{
	...
	return p; // 탐색에 실패했을 경우 NULL을 반환함. 
}
...
{
	if (node == NULL)
		return new_node(key); // 트리가 공백이면 새로운 노드를 반환함. 
...
// 이진 탐색 트리와 키가 주어지면 키가 저장된 노드를 삭제하고 새로운 루트 노드를 반환함. 
...
//이진 탐색 트리를 사용하는 영어 사전 프로그램의 메인 함수 
int main(void)
{
	char command;
	element e;
